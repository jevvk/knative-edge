...
(mkdir -p $TMP/secret \
    && KUBECONFIG=$TMP/kubeconfig-cloud bash e2e/scripts/generate-kubeconfig.sh knative-edge-e2e-edge https://knative-edge-e2e-cloud-control-plane:6443 knative-edge-reflector default > $TMP/secret/kubeconfig \
    && cd $TMP/secret \
